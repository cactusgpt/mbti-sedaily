...
/**
 * 특정 월의 일별 읽기 데이터 가져오기
 */
export function getMonthlyReadings(year: number, month: number): Map<number, number> {
  const data = getStorageData();
 ...
 * 오늘 읽은 기사 수 가져오기
 */
export function getTodayReadCount(): number {
  const data = getStorageData();
  const today = getTodayDate();
  const todayReading = data.dailyReadings.find(r => r.date === today);
  return todayReading?.count || 0;
}

/**
 * 주간 목표 달성률 계산 (목표: 주 7개 기사)
 */
export function getWeeklyProgress(goal: number = 7): number {
  const data = getStorageData();
  const weekStart = getWeekStart();
  const weekArticles = data.dailyReadings
    .filter(r => r.date >= weekStart)
    .reduce((sum, r) => sum + r.count, 0);

  return Math.min(100, Math.round((weekArticles / goal) * 100));
}
