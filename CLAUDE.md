...
**Phase 4-A** (collector scope 축소 + 1면 boost) replaced v2 Collector / Selector schedules from every-3-hour polling (`cron(0 0/3 * * ? *)` / `cron(30 0/3 * * ? *)`) to **once-daily KST midnight** (`cron(0 15 * * ? *)` / `cron(30 15 * * ? *)`). Collector now runs in **paper-mode** by default (DDB feature flag `collector-paper-mode=true`), which (a) defaults its target date to **yesterday KST** because 서울경제 daily-xml/{D}.xml accumulates D+1's 1면 paper articles starting ~17:00 KST and finishing by 23:55 KST, and (b) rejects any `<item>` without a `<paper>` element so the pgvector pool shrinks from ~280 inserts/day to ~50–90 paper-bearing inserts/day. The collector promotes `paper_number` / `paper_date` / `paper_paragraph` (snake_case attrs from v1 `clients/s3_xml_client.py:PaperInfo`, which is **flat — not editing_info/publish_info nested**) into `articles.metadata` (JSONB) so downstream stages can read them without re-parsing the XML. Selector's `composite_score(scores, mbti, paper_number=None)` (`backend/v2/clients/selector_service.py`) adds `+selector-paper-boost-value` (DDB threshold, int default 2) when `paper_number ≤ selector-paper-boost-max-page` (DDB threshold, int default 1) — so 1면 articles naturally land in each MBTI's top-20. Defaults are conservative: boost=2 over a base composite ≤10 means 1면 reliably out-ranks non-1면 even when its raw mbti_score is mid. Roll-back path: toggle `collector-paper-mode` → `false` (DDB `pk=CONFIG`, `sk=feature-flag/collector-paper-mode`, `value={enabled:false}`) — collector falls back to today-KST + no paper filter. boost=0 disables boost without disabling paper-mode. New observability: `collector_paper_mode_run` JSON event in collector logs (target_date / paper_pass / paper_fail_no_element / other_filtered) + `CollectorPaperPass` CloudWatch metric (`sedaily-mbti/v2`, dim `mode=paper|legacy`) emitted via the new generic `emit_count` helper in `clients/cloudwatch_metrics.py`. `selector_run_complete` adds `boosted_articles` and `boost_max_page` fields. Phase 4-A rationale + reconciliation data live in the Phase 4 reconnaissance handoff (P7 daily paper-distribution sample, P8 push-timing histogram). The Phase 4-B agentic-cost work is a separate round.
