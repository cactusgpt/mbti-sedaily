...
**Update (2026-05-10):** main branch 가 force push 로 feature/backend-redesign HEAD (3bcc90a) 와 sync — 이전 saju 분리 후에도 본 repo 에 일별 자동 별자리 publishing 188 commits 가 들어오던 ops 잔재 정리. saju 는 별도 GitHub repo + `saju.sedaily.ai` 도메인으로 완전 분리 (AI LENS/MBTI 와 무관, 사용자 confirm 2026-05-10). 추가로 R1-R7 cleanup commits (2026-05-08 ~ 2026-05-10, 8 commits) 가 root 정리 + CLAUDE.md 분할 적용 — 자세한 내역은 [docs/v2-phase-history.md](docs/v2-phase-history.md).
...
v2 phase history (Phase 4-A / Phase 5 / Cost-3) + repo cleanup history (R1-R7, main force sync 2026-05-10): see [docs/v2-phase-history.md](docs/v2-phase-history.md).
...
Commit trailer must read **exactly**:

```
Co-Authored-By: Claude Opus 4.7 <noreply@anthropic.com>
```

No model-capability suffix like `(1M context)` — GitHub parses author identity from the email, and parentheticals break the author-count stats. (Same rule appears in `backend/v2/.clauderules` as a hard rule.)
...
Note: `README.md` is the GitHub landing-page reference (refreshed 2026-05-10 to track the current Next.js 16 + v1/v2 backend / saju-extracted state). For deeper architecture detail, this CLAUDE.md remains the authoritative source.
