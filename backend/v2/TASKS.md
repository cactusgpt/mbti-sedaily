...
- **Definition of Done**:
  - [ ] 스크립트가 `sedaily-mbti-chat-agent` ECR 리포지토리를 만드는 AWS CLI 명령을 **출력만** 한다 (사람이 복사해서 실행)
  - [ ] `--apply` 플래그가 있을 때만 실제 실행
  - [ ] 리포지토리 이미 존재하면 정상 종료
...
- **Definition of Done**:
  - [ ] `--dry-run` 실행 시 모든 AWS CLI 명령어 출력 (실제 실행 안 함)
  - [ ] `--status` 플래그 구현
  - [ ] `--endpoint` 플래그 구현
  - [ ] **실제 프로비저닝은 사람이 수동 실행** — Claude Code는 건드리지 않는다
