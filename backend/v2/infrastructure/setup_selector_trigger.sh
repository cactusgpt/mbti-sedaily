#!/bin/bash
# =============================================================================
# Register EventBridge rule to invoke Core 1.5 Selector every 3 hours (TASK-4-C).
# =============================================================================
#
...
# for this script — see TASK-4-C handover.
#
# Resources created:
#   1. EventBridge rule `sedaily-mbti-v2-selector-schedule`
#      cron(30 0/3 * * ? *) UTC  →  fires at 00:30/03:30/06:30/09:30/
#                                   12:30/15:30/18:30/21:30 UTC
#                                   = KST 09:30/12:30/15:30/18:30/21:30/
#                                       00:30/03:30/06:30
#      30-min offset behind Collector (cron(0 0/3 * * ? *)) so Collector
#      finishes its raw-article ingest before Selector runs. Collector
#      typical wall-clock ~5–10min for ~100 articles incl. Titan V2
#      embedding; 30min is an ample buffer.
#
#      State: DISABLED at creation. Selector cost is small (Nova Lite
#      ~$0.001 per article batch of 20, ~$0.04/day at 200 articles), but
#      we follow the same "DISABLED until manually enabled" pattern as
#      Transform for consistency and to allow manual invoke verification
#      first. Enable explicitly when ready:
#        ./setup_selector_trigger.sh --enable
#
#   2. Lambda resource-policy statement on `sedaily-mbti-v2-selector-dev`
#      SID=EventBridgeV2SelectorTrigger, action=lambda:InvokeFunction.
#   3. Rule target → Selector Lambda ARN (Id=selector-lambda).
#
# Input: intentionally empty. The Selector handler computes today's KST
# date itself and polls article_selections for unscored raws.
#
# Pipeline scheduling
# -------------------
#   Collector  cron(0 0/3 * * ? *)   → 00:00, 03:00, ... UTC  (every 3h)
#   Selector   cron(30 0/3 * * ? *)  → 00:30, 03:30, ... UTC  (every 3h, +30min)
#   Transform  rate(5 minutes)       → polls selected=TRUE rows continuously
#
# Selector and Transform never fire at the same minute (Selector hits :30,
# Transform hits :00/:05/.../:55) so there's no DB lock contention.
#
# Usage:
#   ./setup_selector_trigger.sh                 — dry-run (default)
#   ./setup_selector_trigger.sh --dry-run       — explicit dry-run
#   ./setup_selector_trigger.sh --apply         — create resources (state=DISABLED)
#   ./setup_selector_trigger.sh --enable        — enable an already-created rule
#   ./setup_selector_trigger.sh --disable       — disable the rule (emergency stop)
#   ./setup_selector_trigger.sh --status        — read-only query of current state
#   ./setup_selector_trigger.sh --help | -h     — this help text
#
...
# ── Configuration ────────────────────────────────────────────────────────────
REGION="us-east-1"
LAMBDA_NAME="sedaily-mbti-v2-selector-dev"
RULE_NAME="sedaily-mbti-v2-selector-schedule"
SCHEDULE_EXPRESSION="cron(30 0/3 * * ? *)"
TARGET_ID="selector-lambda"
PERMISSION_SID="EventBridgeV2SelectorTrigger"
INITIAL_STATE="DISABLED"

RULE_DESCRIPTION="Core 1.5 Selector 3-hour trigger (UTC :30, +30min after Collector). Scores today's raw articles via Nova Lite, upserts article_selections rows, reranks top-20 selected per MBTI. Empty payload — handler resolves today's KST date itself."

# ── Usage helper ─────────────────────────────────────────────────────────────
print_usage() {
  cat <<EOF
setup_selector_trigger.sh — Register EventBridge 3-hour trigger for Core 1.5 Selector.
...
Target: ${RULE_NAME} in ${REGION}
  ...
  input           empty (handler resolves today's KST date)
  initial state   ${INITIAL_STATE} (manual verification first; --enable when ready)
EOF
}
...
if [ -z "$LAMBDA_ARN" ] || [ "$LAMBDA_ARN" = "None" ]; then
  ...
  echo "       TASK-4-C Lambda creation must be complete first (manual via console" >&2
  echo "       per .clauderules #5: VPC + IAM + env vars + initial code stub)." >&2
  exit 1
fi
...
if [ "$MODE" = "enable" ]; then
  ...
  echo "  Now firing every 3 hours (UTC :30). Monitor:"
  echo "    aws logs tail /aws/lambda/$LAMBDA_NAME --since 15m --region $REGION --follow"
  exit 0
fi
...
echo "          Initial state: $INITIAL_STATE (manual verify first; --enable when ready)"
echo ""
...
if [ "$MODE" = "apply" ]; then
  TMP_ERR="$(mktemp -t setup-selector-trigger-perm.XXXXXX)"
...
if [ "$MODE" = "apply" ]; then
  ...
  echo ""
  echo "[NEXT] Rule is DISABLED. To enable 3-hour firing:"
  echo "       ./v2/infrastructure/setup_selector_trigger.sh --enable"
  echo ""
