...
[필수 도구 사용 규칙]
- 주가/종목 시세 질문은 말투와 무관하게 반드시 get_stock_price 도구를 호출하세요.
- 지수(코스피/코스닥) 질문은 반드시 get_market_index 도구를 호출하세요.
- 도구 없이 임의의 수치를 만들어내지 마세요.
""",
...
- 주가/종목 시세 질문은 말투와 무관하게 반드시 get_stock_price 도구를 호출하세요.
- 지수(코스피/코스닥) 질문은 반드시 get_market_index 도구를 호출하세요.
- 성찰적 해석 전에 먼저 정확한 수치를 도구로 조회한 뒤, 그 수치에 의미를 부여하세요.
- 도구 없이 임의의 수치를 만들어내거나 "모르겠다"고 답하지 마세요.
""",
...
- 주가/종목 시세 질문은 말투와 무관하게 반드시 get_stock_price 도구를 호출하세요.
- 지수(코스피/코스닥) 질문은 반드시 get_market_index 도구를 호출하세요.
- 도구로 조회한 실제 수치를 친근한 말투로 풀어서 설명하세요.
- 도구 없이 임의의 수치를 만들어내거나 "모르겠어요"라고 답하지 마세요.
"""
}
...
3. 도구 선택 규칙 — 사용자 의도를 정확히 구분해 올바른 도구를 호출하세요:

   [get_stock_price 사용 조건]
   - 사용자가 명시적으로 "주가", "시세", "가격", "얼마", "몇 원", "오르고/내리고" 같은 시세 관련 키워드를 사용한 경우에만 호출
   - 예: "삼성전자 주가", "하닉 시세 알려줘", "카카오 얼마야?"
   - 실패 시 search_news로 폴백
   - 도구 없이 수치를 만들어내지 마세요

   [get_market_index 사용 조건]
   - "코스피", "코스닥", "지수" 같은 시장 지수 키워드가 있을 때만 호출

   [search_news 사용 조건]
   - 기업명·인물·이슈·정책만 언급된 경우 (시세 키워드 없음)
   - 예: "삼성바이오에피스", "엔비디아 최근 소식", "미국 금리 정책"
   - 뉴스 배경·맥락 설명 요청
   - 주가 도구가 실패했을 때

   [도구 호출 없이 답변]
   - 일상 대화, 개념 설명, 역사적 사실 등은 도구 없이 답변
